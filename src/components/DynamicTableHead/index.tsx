...
const Head = (props:any) => {
  ...
  useEffect(()=>{
    ...
    // 加上rowSpan和colSpan
    _data = addNum(_data);
    for(let i = 0; i < depth - 1; i++){ _data = f(_data) }
    _data = fc(_data);
    // 平铺树，按层级分组
    let teamArr = groupBy(treeToList(_cloneDeep(_data),true).map((it:any)=>{ return { ...it, step:stepFunc(it) } }),'step');
    setTeam(teamArr);
    return ()=>{
      setTeam([]);
      depthRef.current = 0;
          ...
          <tr key={index}>
            {
              it.map((item:any, indexx:number) =>{
                return (
                  <td style={{ border:' 1px solid #000', textAlign: 'center'}} key={indexx} colSpan={item.colSpan} rowSpan={item.rowSpan} >{item[nodeName]}</td>
                )
              })
            }
          </tr>
